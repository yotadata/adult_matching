...
def _load_user_features(db_url: str | None) -> pd.DataFrame | None:
    ...
    sql = (
        ...
        "      filter (where uvd.decision_type in ('swipe_like', 'grid_like')))[:20] as recent_positive_video_ids,"
        "    count(*) filter (where uvd.decision_type in ('swipe_like', 'grid_like') and uvd.created_at >= now() - interval '30 days') as like_count_30d,"
        ...
        "      uvd.user_id,"
        "      vt.tag_id,"
        "      count(*) filter (where uvd.decision_type in ('swipe_like', 'grid_like')) as like_count"
        "    from public.user_video_decisions uvd"
