                ...
                >
                  <div className="w-full rounded-t-2xl bg-white shadow-2xl">
                    ...
                    <div className="h-[90dvh] max-h-[90svh] overflow-hidden flex flex-col">
                      ...
                      {mobileShowFilters && (
                        <div className="p-3 border-b bg-white/90 backdrop-blur-xl">
                          {/* 並び替え */}
                          ...
                          <div className="mb-3">
                            ...
                            <Combobox<TagFilterOption | null> value={null} onChange={(tag) => { if (tag) toggleTag(tag.id); setTagSearch(''); }}>
                              <div className="relative mt-2">
                                ...
                                <Combobox.Options
                                  static
                                  className={buildOptionsClass(tagComboboxOpen, 'max-h-48 overflow-y-auto rounded-2xl border border-gray-200 bg-white/95 shadow-lg z-20')}
                                >
                                  {filteredTagOptions.length === 0 ? (
                                    ...
                                    filteredTagOptions.map((tag) => (
                                      <Combobox.Option
                                        ...
                                        value={tag}
                                        className={({ active }) =>
                                          `flex cursor-pointer items-center justify-between px-3 py-2 text-sm transition ${active ? 'bg-gray-100 text-gray-900' : 'bg-white text-gray-800'}`
                                        }
                                      >
                                        <span>#{tag.name}</span>
                        ...
                        <div className="mb-3">
                            ...
                            >
                              <div className="relative mt-2">
                                ...
                                <Combobox.Options
                                  static
                                  className={buildOptionsClass(performerComboboxOpen, 'max-h-48 overflow-y-auto rounded-2xl border border-gray-200 bg-white/95 shadow-lg z-20')}
                                >
                            ...
                            <div className="space-y-3">
                              {likedVideos.map((video) => (
                                <article
                                  ...
                                  className="rounded-3xl border border-white/60 bg-white/80 p-3 shadow-xl backdrop-blur-xl"
                                >
                                  <div className="relative w-full rounded-2xl overflow-hidden bg-gray-200" style={{ paddingBottom: '70%' }}>
                                    {video.thumbnail_url ? (
                                  ...
                                  <div className="mt-3">
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">{video.series || video.maker || 'シリーズ情報なし'}</p>
                                    {video.product_url ? <p className="text-xs text-gray-400"> {new URL(video.product_url).hostname}</p> : null}
                                  </div>
                                  <div className="mt-3 flex items-center justify-between">
                                    <span className="text-sm font-bold text-[#FF6B81]">
                                    ...
                                    <Link href={toFanzaAffiliate(video.product_url) || '#'} passHref target="_blank" rel="noopener noreferrer">
                                      <button className="rounded-full border border-[#FF6B81] px-3 py-1 text-xs font-bold text-[#FF6B81] transition hover:bg-[#FF6B81] hover:text-white">
                                        見る
                                      </button>
                                    </Link>
                                  </div>
                ...
                >
                  <div className="h-full bg-white/70 backdrop-blur-xl shadow-2xl border border-white/60 rounded-tl-3xl rounded-bl-3xl overflow-hidden">
                    <div className="px-6 py-5 border-b bg-white/90 sticky top-0 z-10 flex items-center justify-between">
                      <div>
                        <Dialog.Title className="text-xl font-bold text-gray-900">いいねした動画</Dialog.Title>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="grid h-[calc(100vh-64px)] grid-cols-1 lg:grid-cols-[270px_1fr] gap-6">
                      <aside className="border-r border-gray-200 bg-white/80 px-6 py-4 space-y-4 overflow-visible flex flex-col">
                            ...
                            <div className="space-y-1">
                              ...
                              >
                                <div className="relative">
                                  ...
                                  <Combobox.Options
                                    static
                                    className={buildOptionsClass(tagComboboxOpen, 'max-h-56 overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-lg z-30')}
                                  >
                                    {filteredTagOptions.length === 0 ? (
                                      <div className="px-3 py-2 text-xs text-gray-400">該当するタグがありません</div>
                                    ) : (
                            ...
                            <div className="space-y-1">
                              ...
                              >
                                <div className="relative">
                                  ...
                                  <Combobox.Options
                                    static
                                    className={buildOptionsClass(performerComboboxOpen, 'max-h-56 overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-lg z-30')}
                                  >
                                    {filteredPerformerOptions.length === 0 ? (
                                      <div className="px-3 py-2 text-xs text-gray-400">該当する出演者がいません</div>
                                    ) : (
                        ...
                        {/* Tags and performers are selected via Combobox above */}
                      </aside>
                      ...
                      <section className="px-6 py-6 overflow-y-auto space-y-4">
                        ...
                        {loading ? (
                          ...
                          <div className="space-y-4">
                            {likedVideos.map((video) => (
                              <article key={video.external_id} className="grid gap-4 rounded-3xl border border-white/60 bg-white/80 p-4 shadow-[0_35px_60px_-40px_rgba(0,0,0,0.5)] backdrop-blur-xl lg:grid-cols-[220px_1fr]">
                                <div className="relative w-full rounded-2xl overflow-hidden bg-gray-200" style={{ paddingBottom: '70%' }}>
                                  {video.thumbnail_url ? (
                                    <Image src={video.thumbnail_url} alt={video.title} fill className="object-cover" />
                                  ) : (
                                ...
                                <div className="flex flex-col justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="mt-1 text-sm text-gray-500">{video.series || video.maker || 'シリーズ情報なし'}</p>
                                    {video.product_url ? <p className="text-xs text-gray-400 mt-1">{new URL(video.product_url).hostname}</p> : null}
                                  </div>
