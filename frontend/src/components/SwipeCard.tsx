      ...
      <div className="flex flex-col text-white p-4 flex-grow">
        ...
        <div className="flex flex-wrap gap-2 my-2">
          {cardData.category.split(' ').map(tag => (
            <span key={tag} className="bg-white/20 text-xs font-semibold px-2 py-1 rounded-full [text-shadow:0_1px_2px_rgba(0,0,0,0.35)]">{tag}</span>
          ))}
        </div>
                <p className="text-sm line-clamp-3 [text-shadow:0_2px_4px_rgba(0,0,0,0.7)]">{cardData.description}</p>
      </div>
    </motion.div>
  );
});
