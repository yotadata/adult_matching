-- get_user_likes に source, image_urls を追加（FANZA_AMATEUR サムネイル高画質化対応）
DROP FUNCTION IF EXISTS public.get_user_likes(text,text,text,integer,integer,uuid[],uuid[],numeric,numeric,timestamptz,timestamptz);
...
RETURNS TABLE(
  ...
  product_url text, price numeric, product_released_at timestamptz,
  liked_at timestamptz, performers jsonb, tags jsonb,
  total_count bigint,
...
GRANT EXECUTE ON FUNCTION public.get_user_likes(text,text,text,integer,integer,uuid[],uuid[],numeric,numeric,timestamptz,timestamptz) TO authenticated;
