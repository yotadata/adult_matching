-- 1. user_video_decisions テーブルの作成
...
-- 2. video_popularity_daily materialized view の DROP (依存関係解消のため)
drop materialized view if exists public.video_popularity_daily;

-- 3. likes から user_video_decisions へのデータ流し込み
-- 既存の likes データを 'like' として新しいテーブルに挿入
insert into public.user_video_decisions (user_id, video_id, decision_type, created_at)
select user_id, video_id, 'like', created_at
from public.likes;

-- 4. likes テーブルの削除
drop table public.likes;

-- 5. video_popularity_daily materialized view の再作成 (新しいテーブルを参照)
