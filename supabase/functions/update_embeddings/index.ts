...
async function updateVideoEmbeddings(
  ...
  // Get videos that need embedding updates
...
async function updateSingleUserEmbedding(supabaseAdmin: any, userId: string) {
  // Fetch user's likes with video data
...
function generateVideoEmbedding(video: any): number[] {
  // Phase 1: Simple item tower implementation
  ...
  // Genre features (100-199)
  if (video.genre) {
    const genreHash = simpleHash(video.genre);
