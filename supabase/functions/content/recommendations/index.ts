...
async function getUserEmbedding(
  ...
  try {
    ...
    // フォールバック：ユーザー行動から特徴量を生成（tags-basedクエリ使用）
    // Direct query for user behavior using tags-based approach
    ...
    if (userBehavior && userBehavior.length > 0) {
      // ユーザー行動から簡易埋め込み生成
      // Generate embedding from tags-based behavior data
      const behaviorData = userBehavior.map(b => {
        ...
        return {
          ...
          videos: {
            genre: genreTag,
            maker: b.videos.maker,
            price: b.videos.price
          }
        };
      });
      return generateUserEmbeddingFromBehavior(behaviorData);
    }
...
async function analyzeUserGenrePreferences(
  ...
  try {
    // Direct query for user genre preferences using tags-based approach
