...
- [ ] CI/CD の PR テスト（`[CiCd] Ops - PR Test Suite`）は `ci:run-tests` ラベルが付いた場合か手動 `workflow_dispatch` のみ走るようにし、テストパス後にマージできる運用をドキュメント化する。
- [ ] UI 全体の用語揺れを防ぐための用語集を `docs/ui/glossary.md` で管理し、ユーザー向けの表記／説明を統一する。
- [ ] アカウント作成時は「18歳以上です」のチェックを必須にし、同画面から利用規約モーダルを開けるリンクを提供する。
- [ ] 利用規約の要点（スクレイピング禁止、個人情報非取得方針、登録の目的など）をユーザーが把握できるよう `docs/features/terms_highlights.md` で整理し、モーダル本文から参照する。
...
- [ ] 好み分析ページ（`frontend/src/app/insights/page.tsx`）では、シェアボタンから結果カードをプレビュー表示し、Twitter(X) 等で共有できる画像 1 枚に利用者の「好きなタグ / 出演者ランキング」や推しポイントを中心に訴求する。
- [ ] 「あなたの性癖 プロフィール」シェアカード DOM を 1920×1080（16:9）固定レイアウトで実装し、既存ダッシュボードと同トーンのグラデ背景＋白カード＋丸角で構成する。ヘッダー（タイトル/サブテキスト/ロゴ）、サマリー指標（気になる数・スキップ数・気になる比率）、好きなタグ/出演者 Top（各上位 N 件、順位バッジ付き）、フッター（集計対象テキスト＋QR コード）を描画し、`html-to-image` 系ライブラリで PNG 出力 → ダウンロード/Share API/X 共有へ連携できるようにする。
