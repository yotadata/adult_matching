...
本ドキュメントは、スワイプ画面と機能が重複しない「AIレコメンド」画面のコンセプト・UX・必要機能を整理したものです。スワイプ UI はリアルタイムな意思決定体験に特化しているため、AIレコメンド画面では「好みの可視化」「テーマ別の深掘り」「プレイリスト化」に主軸を置きます。
...
| カラム | コンテンツ | 目的 |
| ------ | ---------- | ---- |
| 左 | 好みまとめ & プリセット検索 | 最近の好みを見ながらタグ/出演者プリセットを選び、AIに伝える |
| 右 | AIが生成したセッション（最大3ブロック） | 候補作品の比較・選択 |

### 4.1 ヘッダー（全幅）
- タイトル: 「AIで探す」
- サブコピー: 「今日の気分と最近の好みから、最短ルートで作品を見つけましょう。」
- アクション: なし（初期表示でカードをすぐ閲覧できる構成）

### 4.2 左カラム：好みサマリ & プリセット入力
1. **好みサマリカード**
