...
| `NEXT_PUBLIC_SENTRY_DSN` | Sentry DSN（未設定なら Sentry 無効） |
| `SENTRY_ORG` | Sentry 組織スラッグ（ソースマップアップロード用） |
| `SENTRY_PROJECT` | Sentry プロジェクトスラッグ（ソースマップアップロード用） |
| `SENTRY_AUTH_TOKEN` | Sentry 認証トークン（ソースマップアップロード用） |
| `DISCORD_WEBHOOK_URL` | Discord 通知用 Webhook URL（KPI監視・CI通知） |
